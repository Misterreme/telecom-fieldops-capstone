...
function StatCard({ value, label, valueClass = 'text-slate-100' }: StatCardProps) {
  return (
    <div className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 flex flex-col gap-0.5">
      <span className={`text-2xl font-bold leading-none ${valueClass}`}>{value}</span>
      <span className="text-[0.65rem] text-slate-500 uppercase tracking-widest">{label}</span>
    </div>
  );
}
...
function SkeletonCard() {
  return (
    <div className="rounded-xl bg-slate-900 border border-slate-800 p-5 flex flex-col gap-4 animate-pulse">
...
export function PlansPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 px-6 py-8 max-w-7xl mx-auto">

      {/* ── Header ── */}
      <header className="flex items-start justify-between gap-4 mb-8 pb-6 border-b border-slate-800">
        <div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg border border-slate-700 text-slate-400 text-xs font-medium hover:border-sky-500 hover:text-sky-400 transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap shrink-0"
        >
      ...
      <div className="flex flex-col gap-3 mb-5">
        ...
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-9 pr-4 py-2.5 text-sm text-slate-200 placeholder-slate-500 outline-none focus:border-sky-500 transition-colors"
          />
        </div>
      ...
      {loading && plans.length === 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" aria-label="Cargando planes…">
